...
//
////*************** test result *********************************************************************
public class TCreateOrder implements IConstant {
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int index = 0 ; index < maxIndex; index ++) {
				...
				Order order = new Order(DataTest.genOrderNumber(user.getId()));
				order.setUser(user);
				order.setDeliveryAddress(user.getAddressList().get(0));
				order.setBillingAddress(user.getAddressList().get(0));
				order.setBankCardUsed(user.getBankCardList().get(0));
				

				order  = myOrderDao.create(order );
				if (order != null)
					Utils.trace(order.toString());
				
			}

		} catch (Exception e) {
			Utils.trace("catch myOrderDao.create(comment); " + e.toString());
			e.printStackTrace();
		}
